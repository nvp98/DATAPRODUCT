...
{
    public class BM_18_PhanQuyenXiHatController : Controller
    {
        private readonly DataContext _context;

        public BM_18_PhanQuyenXiHatController(DataContext _context)
        {
        ...
        public async Task<IActionResult> Search([FromBody] SearchLoSanXuatDto dto)
        {
            try
            {
                var query = _context.Tbl_BM_18_PhanQuyenXiHat.AsQueryable();
                ...
                var loSanXuatList = await (from lo in query
                                           ...
                                           join map in _context.Tbl_BM_18_PhanQuyenXiHat_TaiKhoan on lo.ID equals map.ID_LoSanXuat into gmap
        ...
        public async Task<IActionResult> SaveOrUpdate([FromBody] LoSanXuatPayloadDTO dto)
        {
            ...
            try
            {
                Tbl_BM_18_PhanQuyenXiHat entity;

                if (dto.ID == null || dto.ID == 0)
                {
                    // Kiểm tra trùng MaLo + ID_BoPhan
                    bool isDuplicate = await _context.Tbl_BM_18_PhanQuyenXiHat
                    ...
                    bool isTenLoDuplicate = await _context.Tbl_BM_18_PhanQuyenXiHat
                    ...
                    // Tạo mới
                    entity = new Tbl_BM_18_PhanQuyenXiHat
                    {
                        TenLo = dto.TenLo,
                        MaLo = dto.MaLo,
                    ...
                    _context.Tbl_BM_18_PhanQuyenXiHat.Add(entity);
                    await _context.SaveChangesAsync(); // cần để có ID
                }
                else
                {
                    // Kiểm tra trùng với bản ghi khác
                    bool isMaLoDuplicate = await _context.Tbl_BM_18_PhanQuyenXiHat
                    ...
                    // Cập nhật
                    entity = await _context.Tbl_BM_18_PhanQuyenXiHat.FindAsync(dto.ID);
                    ...
                    var oldTks = _context.Tbl_BM_18_PhanQuyenXiHat_TaiKhoan.Where(x => x.ID_LoSanXuat == entity.ID);
                    _context.Tbl_BM_18_PhanQuyenXiHat_TaiKhoan.RemoveRange(oldTks);
                }

                // Thêm danh sách tài khoản mới (nếu có)
                if (dto.TaiKhoanIds != null && dto.TaiKhoanIds.Any())
                {
                    var mappings = dto.TaiKhoanIds.Select(id => new Tbl_BM_18_PhanQuyenXiHat_TaiKhoan
                    {
                        ID_LoSanXuat = entity.ID,
                        ID_TaiKhoan = id
                    }).ToList();
                    _context.Tbl_BM_18_PhanQuyenXiHat_TaiKhoan.AddRange(mappings);
                }
        ...
        public async Task<IActionResult> GetById([FromBody] int Id)
        {
            ...
            try
            {
                var item = await (from a in _context.Tbl_BM_18_PhanQuyenXiHat
                ...
                if (item != null)
                {
                    item.ListTaiKhoan = await (from map in _context.Tbl_BM_18_PhanQuyenXiHat_TaiKhoan
