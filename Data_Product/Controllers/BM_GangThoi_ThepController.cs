...
namespace Data_Product.Controllers
{
    public class BM_GangThoi_ThepController : Controller
    {
        ...
        private readonly INhanGangService _nhanGangService;

        public BM_GangThoi_ThepController(DataContext _context, ICompositeViewEngine viewEngine, IChiaGangService chiaGangService, IHRC1LuyenThepService hrc1LuyenThepService, INhanGangService nhanGangService)
        {
            this._context = _context;
            this._chiaGangService = chiaGangService;
            _viewEngine = viewEngine;
            this._hrc1LuyenThepService = hrc1LuyenThepService;
            this._nhanGangService = nhanGangService;
        }
        ...
        public async Task<List<ThungTrungGianGroupViewModel>> GetDanhSachThungTrungGianDaNhan(SearchThungDaNhanDto payload)
        {
            try
            {
                ...
                // ===== 2) TTG + Mẻ thổi (project tối thiểu field cần thiết) =====
                var thungList = await (
                    from ttg in ttgQuery
                    join meThoi in _context.Tbl_MeThoi on ttg.ID_MeThoi equals meThoi.ID into meJoin
                    from meThoi in meJoin.DefaultIfEmpty()
                ...
                var ttgIds = thungList.Select(x => x.ID_TTG).Distinct().ToList();

                // ===== 3) GangLong CHỈ của các TTG đã lọc =====
                var gangListRaw = await _context.Tbl_BM_16_GangLong
                    .Where(x => x.ID_TTG.HasValue && ttgIds.Contains(x.ID_TTG.Value))
                    .Select(x => new
                    {
                        x.ID_TTG,
                ...
                // Fallback seq ổn định
                foreach (var g in gangListRaw)
                    if (!g.Gang.T_ReceiveSeq.HasValue) g.Gang.T_ReceiveSeq = g.Gang.ID;

                var gangIds = gangListRaw.Select(g => g.Gang.ID).Distinct().ToList();

                // ===== 4) Map MaChiaGang mới nhất CHỈ theo gangIds =====
                var lastChiaGangPerThung = await _context.Tbl_BM_16_ChiaGang
                    .Where(c => c.ID_Thung.HasValue && gangIds.Contains(c.ID_Thung.Value))
                ...
                // ===== 5) Đánh dấu IsChiaCR (đã nhận >= 2 lần) CHỈ theo các mã liên quan =====
                var maThungGangSet = gangListRaw.Where(x => !string.IsNullOrEmpty(x.Gang.MaThungGang))
                                                .Select(x => x.Gang.MaThungGang!)
                                                .Distinct(StringComparer.OrdinalIgnoreCase)
                                                .ToList();

                Dictionary<string, int> receiveMap = new(StringComparer.OrdinalIgnoreCase);
                if (maThungGangSet.Count > 0)
                {
                    receiveMap = (await _context.Tbl_BM_16_TaiKhoan_Thung
                ...
                foreach (var g in gangListRaw)
                {
                    var mg = g.Gang.MaThungGang;
                    g.Gang.IsChiaCR = !string.IsNullOrEmpty(mg) && receiveMap.TryGetValue(mg!, out var cnt) && cnt >= 2;
                }

                // ===== 6) Gán danh sách gang vào TTG gốc =====
                var gangByTtg = gangListRaw.GroupBy(x => x.ID_TTG!.Value)
                ...
                // ===== 7) TTG copy: clone hiển thị từ gốc (shallow copy) =====
                ...
                // ===== 8) Nạp phân bổ CR (bản mới nhất cho mỗi cặp GangID, TTG_ID) =====
                var gangIdsAll = thungList
                    .SelectMany(x => x.DanhSachThungGang ?? Enumerable.Empty<GangLongItemViewModel>())
                    .Select(x => x.ID)
                    .Distinct()
                    .ToList();
                if (ttgIds.Count > 0 && gangIdsAll.Count > 0)
                {
                    var keyMaxIds = await _context.Tbl_BM_16_PhanBoGangCR
                            .Where(pb => gangIdsAll.Contains(pb.ID_GangLong)
                                      && ttgIds.Contains(pb.ID_TTG_Target))
                            .GroupBy(pb => new { pb.ID_GangLong, pb.ID_TTG_Target })
                            .Select(g => g.Max(x => x.ID))
                            .ToListAsync();

                    var allocRows = await _context.Tbl_BM_16_PhanBoGangCR
                        .Where(pb => keyMaxIds.Contains(pb.ID))
                        .Select(pb => new
                        {
                            pb.ID_GangLong,
                            pb.ID_TTG_Target,
                    ...
                    var allocCr = allocRows.ToDictionary(k => (k.ID_GangLong, k.ID_TTG_Target), v => v.KL_PhanBo_CR);
                    var allocErr = allocRows.ToDictionary(k => (k.ID_GangLong, k.ID_TTG_Target), v => v.IsSaiChuyenDen);

                    foreach (var ttg in thungList)
                    {
                        decimal sumCR = 0m;
                        var list = ttg.DanhSachThungGang;
                        if (list != null && list.Count > 0)
                        {
                            foreach (var g in list)
                            {
                                var key = (g.ID, ttg.ID_TTG);
                                if (allocCr.TryGetValue(key, out var kl))
                                {
                                    var v = kl ?? 0m;
                                    g.KL_GangChiaCR = v;
                                    sumCR += v;
                                }
                                else g.KL_GangChiaCR = 0m;

                                g.IsSaiChuyenDen = allocErr.TryGetValue(key, out var f) ? f : false;
                            }
                        }
                        ttg.SumKL_GangChiaCR = sumCR;
                    }
                }
                // ===== 9) Sắp trong phạm vi mỗi TTG (anchor-first theo MaChiaGang) =====
                ...
                // ===== 10) Sắp family TTG (copy kề gốc) =====
                var families = thungList
                    .GroupBy(ttg => ttg.MaThungTG ?? string.Empty, StringComparer.OrdinalIgnoreCase)
                    .Select(g =>
                    {
                        var origin = g.FirstOrDefault(x => !x.IsCopy) ?? g.FirstOrDefault();
                        // Nếu vẫn có trường hợp nhóm trống, guard lại:
                        if (origin == null)
                        {
                            return new
                            {
                                Key = g.Key,
                                Origin = (ThungTrungGianGroupViewModel)null,
                                Items = g.ToList(),
                                HasMe = 1,
                                MaMeThoi = (string)null,
                                MinSeq = int.MaxValue,
                                Ngay = (DateTime?)null,
                                BlockKeys = new List<string?>()
                            };
                        }

                        var minSeq =
                            (origin.DanhSachThungGang ?? Enumerable.Empty<GangLongItemViewModel>())
                            .Select(x => x.T_ReceiveSeq ?? int.MaxValue)
                ...
                for (int i = 0; i < famBase.Count; i++)
                {
                    ...
                    string? pivot = f.BlockKeys.FirstOrDefault(k => k != null && dupKeys.Contains(k) && !emittedKey.Contains(k));
                    if (pivot != null)
                    {
                ...
                foreach (var fi in finalFamiliesIdx) finalBlocks.AddRange(famBase[fi].Items);

                return finalBlocks;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ERROR] GetDanhSachThungTrungGianDaNhan: {ex}");
        ...
        public async Task<IActionResult> HuyNhan([FromBody] HuyNhanThungDto payload)
        {
            ...
            try
            {
                ...
                var snapshotToHops = await _context.Tbl_BM_16_ThungTrungGian
                ...
                var deletedTTGSnapshots = await _context.Tbl_BM_16_ThungTrungGian
                    .Where(t => ID_TTGList.Contains(t.ID))
                    .Select(t => new
                    {
                        t.ID,
                        t.NoiNhan,
                        t.SoThungTG,
                        MaThungGangList = _context.Tbl_BM_16_GangLong
                            .Where(gl => gl.ID_TTG == t.ID)
                            .Select(gl => gl.MaThungGang)
                            .Distinct()
                            .ToList()
                    })
                ...
                var deletedTTGIds = new List<int>();
                var changedTTGIds = new HashSet<int>();

                var affectedThungIds = new HashSet<int>();
                    ...
                    if (!otherThungGang.Any())
                    {
                        ...
                        if (thungTG != null)
                        {
                            var copies = await _context.Tbl_BM_16_ThungTrungGian
                                ...
                                .ToListAsync();
                            deletedTTGIds.Add(thungTG.ID);

                            foreach (var c in copies)
                            {
                                deletedTTGIds.Add(c.ID);
                            }
                ...
                foreach (var thung in thungGocCanXet)
                {
                    ...
                    else
                    {
                        ...
                        var listThungThepCanXoa = thungCopyCanXoa.Select(x => x.MaThungThep).ToList();
                        var thungCopy = await _context.Tbl_BM_16_GangLong
                            .Where(x => x.MaThungGang == thung.MaThungGang && x.T_copy == true && !listThungThepCanXoa.Contains(x.MaThungThep))
                            .OrderBy(x => x.NgayLuyenThep)
                            .FirstOrDefaultAsync();

                        if (thungCopy != null)
                        {
                            affectedThungIds.Add(thung.ID);       // gốc bị xóa
                ...
                foreach (var c in thungCopyCanXoa)
                {
                    affectedThungIds.Add(c.ID);
                }
                ...
                var affectedTTGIds = new HashSet<int>();

                foreach (var snap in snapshotToHops)
                {
                    if (snap.GangSet == null || snap.GangSet.Count == 0)
                        continue;

                    var ids = await _context.Tbl_BM_16_ThungTrungGian
                        .Where(t =>
                            t.NoiNhan == snap.NoiNhan &&
                            t.SoThungTG == snap.SoThungTG &&
                            _context.Tbl_BM_16_GangLong.Any(gl =>
                                gl.ID_TTG == t.ID &&
                ...
                if (affectedTTGIds.Any())
                {
                    ...
                    foreach (var id in affectedTTGIds)
                        table.Rows.Add(id);
                ...
                var affectedThungIdList = affectedThungIds.ToList();
                await this._hrc1LuyenThepService.HuyMocNoi(affectedThungIdList);
        ...
        private async Task XuLyNhietDo_HRC1(List<ThungTrungGianDto> ds)
        {
            ...
            foreach (var group in groups)
            {
                ...
                // Lấy nhiệt độ từ thùng đầu tiên có NhietDo trong nhóm
            ...
            foreach (var item in noGroup)
            {
                ...
                if (dbRow != null)
                    dbRow.NhietDo = thung.NhietDo;
            }
        }

        private async Task XuLyNhietDo_HRC2(List<ThungTrungGianDto> ds)
        {
            ...
            foreach (var tg in dsCoChia)
            {
                ...
                // 3) Lấy nhiệt độ của thùng đầu tiên không null
        ...
        public async Task<IActionResult> Luu([FromBody] List<ThungTrungGianDto> dsThungTG)
        {
            ...
            foreach (var tgDto in dsThungTG)
            {
                ...
                else
                {
                    ...
                    // ============================
                    ttg = await _context.Tbl_BM_16_ThungTrungGian
                        .FirstOrDefaultAsync(x => !x.IsCopy && x.MaThungTG == tgDto.MaThungTG);

                    if (ttg == null)
                        continue;
                }
                // Kiểm tra thùng gang đã chốt
                bool coGangDaChot = await _context.Tbl_BM_16_GangLong
                    .AnyAsync(x => x.ID_TTG == ttg.ID && x.ID_TrangThai == (int)TinhTrang.DaChot);

                if (coGangDaChot)
                {
                    danhSachBiBoQua.Add(tgDto.IsCopy ? tgDto.MaThungTG_Copy : tgDto.MaThungTG);
                    continue;
                }

                //ttgEntitiesCanTinh.Add(ttg);
            ...
            // Sau cùng tính toán lại các thùng gang
        ...
        IEnumerable<(string NoiNhan, string SoThungTG, string GroupKey)> inputKeys)
        {
            var keySet = inputKeys
                .Select(x => $"{x.NoiNhan}|{x.SoThungTG}|{x.GroupKey}")
                .ToHashSet();
            ...
            // 3️⃣ Group + build GroupKey rồi lọc CHÍNH XÁC
            return raw
                ...
                .Select(g => (
                    ...
                    g.Key.NoiNhan,
                    g.Key.SoThungTG,
                    GroupKey: BuildGroupKey(g.Select(x => x.MaThungGang))
                ))
                .Where(x => keySet.Contains(
                    $"{x.NoiNhan}|{x.SoThungTG}|{x.GroupKey}"
                ))
                .ToList();
        }
        ...
        [HttpPost]
        public async Task<IActionResult> GopThungGang([FromBody] GopThungGangDto payload)
        {
            try
            {
                ...
                //await _context.SaveChangesAsync();
                var TenTaiKhoan = User.FindFirstValue(ClaimTypes.Name);
                await _chiaGangService.GopThungGangAsync(payload, TenTaiKhoan);
                return Ok(new { success = true });
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> ExportToExcel([FromBody] SearchThungDaNhanDto payload)
        {
            try
            {
                ...
                // Cột dữ liệu
                const int COL_NHIETDOGANGVAOHRC = 6;
                const int COL_NHIET_DO = 7;
                const int COL_KL_VA_GANG = 8;
                const int COL_KL_THUNG = 9;
                const int COL_KL_GANGLONG = 10;
                const int COL_KLGANGCHIA = 11;   // Không merge

                // Cột cấp TTG (13..21)
                int[] TTG_COLS = {  14, 15, 16, 17, 18, 19, 20, 21, 22 };
                ...
                using (var workbook = new XLWorkbook(filePath))
                {
                    ...
                    void FinalizeLocalGroup_HRC2()
                    {
                        ...
                        {
                            ws.Cell(start, COL_NHIETDOGANGVAOHRC).Value = ws.Cell(firstValueRow, COL_NHIETDOGANGVAOHRC).Value;
                        ...
                        // Merge 6–8
                        ws.Range(start, COL_NHIETDOGANGVAOHRC, end, COL_NHIETDOGANGVAOHRC).Merge();
                    ...
                    foreach (var ttg in thungList)
                    {
                        ...
                        foreach (var item in danhSach)
                        {
                            ...
                            ws.Cell(row, c++).Value = item.BKMIS_SoMe;          //2
                            ws.Cell(row, c++).Value = item.MaThungThep;       // 3
                            ws.Cell(row, c++).Value = item.ID_LoCao;          // 4
                            ws.Cell(row, c++).Value = item.BKMIS_ThungSo;     // 5
                            ...
                            if (ttg.IsCopy == true)
                            {
                                ...
                                ws.Cell(row, c++).Value = ""; // 10
                                ws.Cell(row, c++).Value = ""; // 11 (KLGangChia - không merge)
                                ws.Cell(row, c++).Value = ""; // 12 KL chia CR ( không merge)
                            }
                            else
                            {
                                ws.Cell(row, c++).Value = item.NhietDoGangVaoHRC; // 6
                                ws.Cell(row, c++).Value = item.NhietDo; //7
                                ws.Cell(row, c++).Value = item.T_KLThungVaGang; // 8
                                ws.Cell(row, c++).Value = item.T_KLThungChua;   // 9

                                var cellGangLong = ws.Cell(row, c++);           // 10
                                ...
                                var cellChiaCR = ws.Cell(row, c++);               // 12
                        ...
                        ws.Cell(r1, col).Value = thungDauTien.KLXiKR; ws.Range(r1, col, r2, col).Merge();
                    ...
                    if (isHRC1)
                    {
                        ...
                        while (idx < allRows.Count)
                        {
                            ...
                            if (r2 > r1)
                            {
                                ...
                                {
                                    ws.Cell(r1, COL_NHIETDOGANGVAOHRC).Value = ws.Cell(firstNonCopyRow, COL_NHIETDOGANGVAOHRC).Value;
                                ...
                                // Merge 6–8
                                ws.Range(r1, COL_NHIETDOGANGVAOHRC, r2, COL_NHIETDOGANGVAOHRC).Merge();
                    ...
                    // Dòng tổng
                    int sumRow = row;
                    var totalLabel = ws.Range($"A{sumRow}:J{sumRow}").Merge();
                    ...
                    var totalLabel2 = ws.Range($"M{sumRow}:N{sumRow}").Merge();
                    totalLabel2.Value = "";
                    ...
                    var totalLabel3 = ws.Range($"P{sumRow}:S{sumRow}").Merge();
                    totalLabel3.Value = "";
                    totalLabel3.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left;

                    ws.Cell(sumRow, 20).FormulaA1 = $"=SUM(T8:T{row - 1})";
                    ws.Cell(sumRow, 20).Style.NumberFormat.Format = "#,##0.00";
                    ws.Cell(sumRow, 20).Style.Font.SetBold();
                    ws.Cell(sumRow, 20).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Right;

                    var totalLabel4 = ws.Range($"U{sumRow}:X{sumRow}").Merge();
                    totalLabel4.Value = "";
                    totalLabel4.Style.Font.SetBold();
                    totalLabel4.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Left;

                    // Format chung
                    var usedRange = ws.Range($"A7:X{sumRow}");
        ...
        [HttpPost]
        public async Task<IActionResult> LoadDanhSachThungThepHRC1AT([FromBody] LoadDanhSachThungThepHRC1ATDto payload)
        {
            try
            {
        ...
        [HttpPost]
        public async Task<IActionResult> MocNoiThung([FromBody] MocNoiThung payload)
        {
            //if (payload == null || payload.selectedThungs == null || payload.selectedThungs.Count == 0)
            //    return BadRequest("Danh sách thùng trống.");
            //if (payload.idCa <= 0 || payload.idLoThoi <= 0 || payload.idNguoiNhan <= 0)
            ...
            try
            {
                ...
                var tenTaiKhoan = User.FindFirstValue(ClaimTypes.Name);
                await this._nhanGangService.ExecuteAsync(payload, tenTaiKhoan);
                return Ok(new { Message = "Đã xử lý nhận thùng."});
            }
            catch (Exception ex)
            {
                return StatusCode(500,  ex.Message);
