            ...
            try
            {
                ...
                if (ttgIdsAll.Count > 0 && gangIdsAll.Count > 0)
                {
                    var allocRows = await _context.Tbl_BM_16_PhanBoGangCR
                        ...
                        .Select(pb => new { pb.ID_GangLong, pb.ID_TTG_Target, pb.KL_PhanBo_CR })
                        .AsNoTracking()
                        .ToListAsync();

                    var allocDict = allocRows.ToDictionary(
                    ...
                    foreach (var ttg in thungList)
                    {
                        ...
                        if (ttg.DanhSachThungGang != null && ttg.DanhSachThungGang.Count > 0)
                        {
                            ...
                            {
                                if (allocDict.TryGetValue((g.ID, ttg.ID_TTG), out var kl)) // kl: decimal?
                                {
                                    var val = kl ?? 0m;           // hoặc kl.GetValueOrDefault()
                                    g.KL_GangChiaCR = val;        // decimal
                                    sumCR += val;                 // decimal
                                }
                                else
                                {
