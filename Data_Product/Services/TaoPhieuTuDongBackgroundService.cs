...
{
    public class TaoPhieuTuDongJob : IJob
    {
        private readonly DataContext _context;
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<TaoPhieuTuDongJob> _logger;
        private readonly IConfiguration _configuration;

        public TaoPhieuTuDongJob(IServiceProvider serviceProvider, ILogger<TaoPhieuTuDongJob> logger, IConfiguration configuration)
        {
    ...
    private async Task TaoPhieuVaNapThungAsync()
    {
        ...
        for (int idLoCao = 1; idLoCao <= 5; idLoCao++)
        {
            ...
            // XÓA thùng đã có trong DB nhưng không còn trong dữ liệu BKMIS
            foreach (var thungDb in thungDaCoDict.Values)
            {
                if (!soMeMoi.Contains(thungDb.BKMIS_SoMe))
                {
                    if (thungDb.G_ID_TrangThai == 1 && thungDb.ID_TrangThai == 2 && (thungDb.XacNhan == false || thungDb.XacNhan == null))
                    {
                        _context.Tbl_BM_16_GangLong.Remove(thungDb);
                        GhiLogFile($"[INFO] Đã xoá thùng không còn trong BKMIS: {thungDb.BKMIS_SoMe}");
                    }
                    else
                    {
                        GhiLogFile($"[INFO] Bỏ qua không xoá thùng (vì đã xử lý): {thungDb.BKMIS_SoMe}");
                    }
                }
            }
            ...
            foreach (var thung in danhSachThung)
            {
                ...
                {
                    if (thungDaCo.G_ID_TrangThai == 1 && thungDaCo.ID_TrangThai == 2 && (thungDaCo.XacNhan == false || thungDaCo.XacNhan == null))
                    {
                    ...
                    {
                        _logger.LogInformation("Bỏ qua thùng : {code}", thung.TestPatternCode);
                        GhiLogFile($"[INFO] Bỏ qua thùng : {thung.TestPatternCode}");
                    }
                    continue;
                }
